...
def is_supported_music_url(value: str) -> bool:
    return (
...
def setup() -> None:
    cfg = load_config()
    print("=" * 56)
    print("  Flow Switch - free MVP setup")
    print("=" * 56)
    print()
    print("請先在你的音樂服務中準備好 4 個 playlist 或 album，然後把連結貼進來。")
    ...
    for key, label, hint in MODES:
        ...
        while True:
            value = input(f"貼上 {label} playlist 或 album URL: ").strip()
            ...
            if not value:
                ...
                cfg.pop(key, None)
                break
            if is_supported_music_url(value):
                cfg[key] = {"label": label, "url": value, "hint": hint}
                break
            print("  看起來不像支援的音樂連結，請貼 open.spotify.com 或 spotify: 開頭的連結。")
        print()
...
def start() -> None:
    ...
    if not ready:
        print("還沒有任何手動音樂連結設定。請先雙擊 4_manual_setup.command。")
        sys.exit(1)

    print("=" * 56)
    print("  Flow Switch - free MVP")
    print("=" * 56)
    print()
    ...
    print(f"正在打開音樂連結：{label}")
    webbrowser.open(url)
    print("已送出打開請求。打開後請在音樂 app 裡手動按播放。")
